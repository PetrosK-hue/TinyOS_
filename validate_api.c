...
BOOT_TEST(test_join_illegal_tid_gives_error,
	"Test that ThreadJoin rejects an illegal Tid")//1
{
...
BOOT_TEST(test_detach_illegal_tid_gives_error,
	"Test that ThreadDetach rejects an illegal Tid")//2
{
...
BOOT_TEST(test_detach_self,
	"Test that a thread can detach itself")//3
{
...
BOOT_TEST(test_detach_other,
	"Test that a thread can detach another thread.")//4
{
...
BOOT_TEST(test_multiple_detach,
	"Test that a thread can be detached many times.")//5
{
	ASSERT(ThreadDetach(ThreadSelf())==0);
	ASSERT(ThreadDetach(ThreadSelf())==0);
	ASSERT(ThreadDetach(ThreadSelf())==0);
...
BOOT_TEST(test_join_main_thread,
	"Test that the main thread can be joined by another thread")//6
{
...
BOOT_TEST(test_detach_main_thread,
	"Test that the main thread can be detached")//6
{
	Tid_t mttid;
	...
	int main_thread(int argl, void* args) {
		...
		sleep_thread(1);
		ASSERT(ThreadDetach(ThreadSelf())==0);
		sleep_thread(1); // EXTRA THREAD SLEEP IN ORDER TO PASS THE DETACHED THREAD
		return 42;
	}
...
BOOT_TEST(test_detach_after_join,
	"Test that a thread can be detached after joining")//7
{
	/* A thread to be joined */
	int joined_thread(int argl, void* args) {
		sleep_thread(1);
...
BOOT_TEST(test_cyclic_joins,
	"Test that a set of cyclically joined threads will not deadlock once the cycle breaks")//11
{
